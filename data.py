...
def getEdge(positions,radius,selfConn=True):
    _tree = nb.KDTree(positions)
    _receiverList,_distance = _tree.query_radius(positions,r=radius,return_distance=True)
    _nNodes = len(positions)
    _senders = np.repeat(range(_nNodes),[len(_r) for _r in _receiverList])
    _receivers = np.concatenate(_receiverList,axis=0)
    _attrs = np.concatenate(_distance,axis=0)
    _attrs = np.exp(-K*_attrs)
    _edgeIdx = torch.LongTensor(np.array([_senders,_receivers]))
    _edgeAttr = torch.DoubleTensor(_attrs)
